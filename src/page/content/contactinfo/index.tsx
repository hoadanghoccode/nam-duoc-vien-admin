...
const ContactInfoPage: React.FC = () => {
  ...
  const fetchContent = async () => {
    try {
      setLoading(true);
      const response = await contentApi.getContentPage(PAGE_KEY_MAP.CONTACT_INFO);
  ...
  const handleSave = async (content: string, title: string) => {
    try {
      await contentApi.updateContentPage(PAGE_KEY_MAP.CONTACT_INFO, {
        pageTitle: title,
        contentHTML: content,
        isActive: true
      });
  ...
  return (
    <TinyMCEEditor
      pageKey={PAGE_KEY_MAP.CONTACT_INFO}
      pageTitle={contentData?.pageTitle || "Thông tin liên hệ"}
