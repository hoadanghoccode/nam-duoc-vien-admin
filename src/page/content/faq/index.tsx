...
const FAQPage: React.FC = () => {
  ...
  const fetchContent = async () => {
    try {
      setLoading(true);
      const response = await contentApi.getContentPage(PAGE_KEY_MAP.FAQ);
  ...
  const handleSave = async (content: string, title: string) => {
    try {
      await contentApi.updateContentPage(PAGE_KEY_MAP.FAQ, {
        pageTitle: title,
        contentHTML: content,
        isActive: true
      });
  ...
  return (
    <TinyMCEEditor
      pageKey={PAGE_KEY_MAP.FAQ}
      pageTitle={contentData?.pageTitle || "Câu hỏi thường gặp"}
