...
const PrivacyPolicyPage: React.FC = () => {
  ...
  const fetchContent = async () => {
    try {
      setLoading(true);
      const response = await contentApi.getContentPage(PAGE_KEY_MAP.PRIVACY_POLICY);
  ...
  const handleSave = async (content: string, title: string) => {
    try {
      await contentApi.updateContentPage(PAGE_KEY_MAP.PRIVACY_POLICY, {
        pageTitle: title,
        contentHTML: content,
        isActive: true
      });
  ...
  return (
    <TinyMCEEditor
      pageKey={PAGE_KEY_MAP.PRIVACY_POLICY}
      pageTitle={contentData?.pageTitle || "Chính sách bảo mật"}
