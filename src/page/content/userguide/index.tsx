...
const UserGuidePage: React.FC = () => {
  ...
  const fetchContent = async () => {
    try {
      setLoading(true);
      const response = await contentApi.getContentPage(PAGE_KEY_MAP.USER_GUIDE);
  ...
  const handleSave = async (content: string, title: string) => {
    try {
      await contentApi.updateContentPage(PAGE_KEY_MAP.USER_GUIDE, {
        pageTitle: title,
        contentHTML: content,
        isActive: true
      });
  ...
  return (
    <TinyMCEEditor
      pageKey={PAGE_KEY_MAP.USER_GUIDE}
      pageTitle={contentData?.pageTitle || "Hướng dẫn sử dụng"}
